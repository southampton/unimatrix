...
(function (factory) {
    ...
    $("a", ctx._pfFilter.activeFilterControls).last().on("click", function (e) {
      ...
      for (i = 0; i < ctx._pfFilter.filters.length; i++) {
        if (ctx._pfFilter.filters[i].column === filter.column && ctx._pfFilter.filters[i].value === filter.value) {
          ...
          compileSearch(dt);
          //dt.column(filter.column).search('')
  ...
  function compileSearch(dt) {
    var ctx = dt.settings()[0];
    dt.columns().every(function() { this.search('')});
    for (var i = 0; i < ctx._pfFilter.filters.length; i++) {
      if (dt.column(ctx._pfFilter.filters[i].column).search() !== '') {
        dt.column(ctx._pfFilter.filters[i].column).search(dt.column(ctx._pfFilter.filters[i].column).search().replace('\\', '\\\\').replace('|', '\\|') + '|' + ctx._pfFilter.filters[i].value.replace('\\', '\\\\').replace('|', '\\|'), true);
      } else {
        dt.column(ctx._pfFilter.filters[i].column).search(dt.column(ctx._pfFilter.filters[i].column).search().replace('\\', '\\\\').replace('|', '\\|') + ctx._pfFilter.filters[i].value.replace('\\', '\\\\').replace('|', '\\|'), true);
      }
  ...
  function addFilter (dt, filter) {
    ...
    if (!found) {
      ...
      console.log(filter);
      compileSearch(dt);
      //dt.column(filter.column).search(filter.value);
      dt.one('draw', function() {
	      updateFilterResults(dt);
      });
      dt.draw();
  ...
  function updateFilterResults (dt) {
    var ctx = dt.settings()[0];
    var filteredRows = dt.rows().count();
